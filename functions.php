...
function _load_functions( $folder ): void
{
    ...
    foreach ( $files as $file ) {
        ...
        if( is_dir( $path ) )
            _load_functions( $path );
        else
            require_once $path;
    }
}

_load_functions( get_template_directory() . "/functions" );
...
function getColoredText( string $text, array $params = [] ): string
{
    foreach ( $params as $key => $attr ) {
        if( $key === "{" )
            $text = str_replace("{$key}", "<span class='{class}'>", $text);

        foreach ( $attr as $k => $v ) {
            if( is_array($v) ) {
                $text = str_replace("{{$k}}", implode(" ", $v), $text);
            }

            if( $k === "end" )
                $text = str_replace("{$v}", "</span>", $text);
        }
    }
